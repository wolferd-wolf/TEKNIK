...
func _init() -> void:
	...
	if _failures == 0:
		print(
			"PHASE_ONE_ITEM_TESTS_PASS items=", ItemRegistry.phase_one_items().size(),
...
func _test_catalog() -> void:
	var expected: Array[StringName] = [
		ItemRegistry.ITEM_ANDESITE_ALLOY,
		ItemRegistry.ITEM_ZINC_INGOT,
		ItemRegistry.ITEM_BRASS_INGOT,
		ItemRegistry.ITEM_COPPER_SHEET,
		ItemRegistry.ITEM_BRASS_SHEET,
		ItemRegistry.ITEM_IRON_SHEET,
		ItemRegistry.ITEM_GOLD_SHEET,
		ItemRegistry.ITEM_ANDESITE_CASING,
		ItemRegistry.ITEM_BRASS_CASING,
		ItemRegistry.ITEM_SHAFT,
		ItemRegistry.ITEM_COGWHEEL,
		ItemRegistry.ITEM_LARGE_COGWHEEL,
		ItemRegistry.ITEM_BELT_CONNECTOR,
		ItemRegistry.ITEM_MECHANICAL_BEARING,
		ItemRegistry.ITEM_HAND_CRANK,
		ItemRegistry.ITEM_WRENCH,
		ItemRegistry.ITEM_ELECTRON_TUBE,
		ItemRegistry.ITEM_PRECISION_MECHANISM,
	...
	_expect(ItemRegistry.registered_items().size() == 36, "catalog fits the expanded inventory exactly")
	_expect(ItemRegistry.placeable_items().size() == 8, "stable eight-slot hotbar remains unchanged")
...
func _test_inventory_migration_and_capacity() -> void:
	...
	_expect(migrated.slots().size() == StackInventory.SLOT_COUNT, "migration expands to thirty-six slots")
	_expect(migrated.count(ItemRegistry.ITEM_STONE) == 12, "migration preserves stone")
	_expect(migrated.count(ItemRegistry.ITEM_WORKBENCH) == 1, "migration preserves engineering items")

	var full := StackInventory.new()
	for item_id: StringName in ItemRegistry.registered_items():
		_expect(full.add(item_id, 1) == 0, "catalog item fits inventory: " + str(item_id))
	_expect(full.slots().size() == 36, "inventory exposes thirty-six slots")
	var encoded: Dictionary = full.encode()
	_expect(int(encoded.schema) == StackInventory.SCHEMA, "expanded inventory writes schema two")
...
func _test_progression_chain() -> void:
	...
	_expect(progression.is_unlocked(ProgressionState.UNLOCK_WORKBENCH), "workbench unlock granted")
