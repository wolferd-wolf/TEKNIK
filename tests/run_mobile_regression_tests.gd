...
func _test_shipping_scene() -> void:
	...
	var survival_shipping_source: String = FileAccess.get_file_as_string("res://src/main/survival_shipping_main.gd")
	var engineering_source: String = FileAccess.get_file_as_string("res://src/main/engineering_progression_main.gd")
	var survival_source: String = FileAccess.get_file_as_string("res://src/main/survival_main.gd")
	var multi_lod_source: String = FileAccess.get_file_as_string("res://src/main/multi_lod_main.gd")
	var collision_source: String = FileAccess.get_file_as_string("res://src/main/incremental_collision_main.gd")
	var movement_source: String = FileAccess.get_file_as_string("res://src/main/movement_streaming_main.gd")
	_expect(
		scene_text.contains("kinetic_machine_main.gd")
		and kinetic_source.contains("survival_shipping_main.gd")
		and survival_shipping_source.contains("engineering_progression_main.gd")
		and engineering_source.contains("survival_main.gd")
		and survival_source.contains("multi_lod_main.gd")
		and multi_lod_source.contains("incremental_collision_main.gd")
		and collision_source.contains("movement_streaming_main.gd")
		and movement_source.contains("biome_visual_main.gd"),
		"shipping scene reaches biome visuals through the complete kinetic survival stack"
	)
	var biome_main_source: String = FileAccess.get_file_as_string("res://src/main/biome_visual_main.gd")
	_expect(
...
func _test_feature_planner() -> void:
	...
	var gameplay_source: String = FileAccess.get_file_as_string("res://src/main/procedural_gameplay_main.gd")
	var stream_source: String = FileAccess.get_file_as_string("res://src/main/procedural_gameplay_main.gd_stream.gd")
	_expect(gameplay_source.contains("Thread.new()"), "vegetation planning runs outside the render thread")
	_expect(gameplay_source.contains("ECOLOGY_GROUPS_PER_FRAME"), "vegetation batches use a per-frame commit budget")
	_expect(gameplay_source.contains("old_features_visible"), "base ecology replacement retains compatibility diagnostics")
	_expect(
	...
	_expect(
		...
		or FileAccess.get_file_as_string("res://src/qa/gameplay_capture_director.gd").contains("QA_ECOLOGY_STREAM_PASS"),
...
func _test_biome_region_field() -> void:
	...
	_expect(first.distance_to(nearby) < 0.24, "nearby samples stay inside coherent broad biome regions")
...
func _test_biome_surface_palette() -> void:
	...
	for world_z: int in range(-768, 769, 64):
		for world_x: int in range(-768, 769, 64):
			...
			var weights: Vector4 = BiomePalette.biome_weights(seed, world_x, world_z, float(height))
