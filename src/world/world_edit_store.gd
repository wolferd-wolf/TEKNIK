...
static func chunk_coordinate(world_position: Vector3i) -> Vector3i:
	return Vector3i(floori(float(world_position.x) / float(VoxelChunk.SIZE)), floori(float(world_position.y) / float(VoxelChunk.SIZE)), floori(float(world_position.z) / float(VoxelChunk.SIZE)))
...
func has_override(world_position: Vector3i) -> bool:
	var coordinate: Vector3i = chunk_coordinate(world_position)
	return _chunks.has(coordinate) and (_chunks[coordinate] as Dictionary).has(VoxelChunk.index_of(local_coordinate(world_position)))
...
func snapshot_neighborhood(coordinate: Vector3i) -> Dictionary:
	var snapshots: Dictionary = {}
	var offsets: Array[Vector3i] = [Vector3i.ZERO, Vector3i.LEFT, Vector3i.RIGHT, Vector3i.DOWN, Vector3i.UP, Vector3i(0, 0, -1), Vector3i(0, 0, 1)]
	for offset: Vector3i in offsets:
		var neighbor: Vector3i = coordinate + offset
		var edits: Dictionary = snapshot_for_chunk(neighbor)
		if not edits.is_empty():
			snapshots[neighbor] = edits
	return snapshots


func edited_chunk_coordinates() -> Array[Vector3i]:
...
func encode(seed: int) -> Dictionary:
	var coordinates: Array[Vector3i] = edited_chunk_coordinates()
	...
	for coordinate: Vector3i in coordinates:
		...
		for index_variant: Variant in edits.keys(): indices.append(int(index_variant))
		indices.sort()
		var values: Array[int] = []
		for index: int in indices: values.append(int(edits[index]))
		chunk_records.append({"coordinate": [coordinate.x, coordinate.y, coordinate.z], "indices": indices, "materials": values})
	return {"schema": SCHEMA_VERSION, "world_seed": seed, "chunk_size": VoxelChunk.SIZE, "chunks": chunk_records}


func decode(document: Dictionary, expected_seed: int) -> Error:
	if int(document.get("schema", -1)) != SCHEMA_VERSION: return ERR_FILE_UNRECOGNIZED
	if int(document.get("chunk_size", -1)) != VoxelChunk.SIZE: return ERR_FILE_CORRUPT
	if int(document.get("world_seed", expected_seed)) != expected_seed: return ERR_INVALID_DATA
	var rebuilt: Dictionary = {}
	var records: Array = document.get("chunks", [])
	for record_variant: Variant in records:
		if not record_variant is Dictionary: return ERR_FILE_CORRUPT
		...
		if coordinate_values.size() != 3 or indices.size() != materials.size(): return ERR_FILE_CORRUPT
		...
		for index: int in range(indices.size()):
			...
			if voxel_index < 0 or voxel_index >= VoxelChunk.VOLUME or material < 0 or material > 255: return ERR_FILE_CORRUPT
			edits[voxel_index] = material
		if not edits.is_empty(): rebuilt[coordinate] = edits
	_chunks = rebuilt
	_dirty = false
	return OK
...
func save_atomic(path: String, seed: int) -> Error:
	...
	if rename_error == OK: _dirty = false
	return rename_error


func load_file(path: String, expected_seed: int) -> Error:
	var absolute_path: String = path if path.is_absolute_path() else ProjectSettings.globalize_path(path)
	if not FileAccess.file_exists(absolute_path): return ERR_FILE_NOT_FOUND
