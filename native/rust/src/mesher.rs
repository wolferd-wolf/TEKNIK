...
const DIRECTION_SHIFT: u32 = 18;
const MATERIAL_SHIFT: u32 = 21;
...
#[inline]
fn plane_origin_is_valid(plane_origin: [u32; 3], direction: u32) -> bool {
    let Some((axis, _)) = direction_axis(direction) else {
        return false;
    };
    plane_origin.into_iter().enumerate().all(|(coordinate_axis, value)| {
...
fn pack_face(
    ...
    debug_assert!(plane_origin_is_valid(plane_origin, direction));
    debug_assert!(material > AIR);
    debug_assert!((1..=SIZE as u32).contains(&width));
    debug_assert!((1..=SIZE as u32).contains(&height));
    debug_assert!(flags <= THREE_BIT_MASK);
    TeknikPackedFace {
        geometry: ((plane_origin[0] & SIX_BIT_MASK) << X_SHIFT)
            | ((plane_origin[1] & SIX_BIT_MASK) << Y_SHIFT)
            | ((plane_origin[2] & SIX_BIT_MASK) << Z_SHIFT)
            | ((direction & THREE_BIT_MASK) << DIRECTION_SHIFT)
            | (((material as u32) & EIGHT_BIT_MASK) << MATERIAL_SHIFT)
            | ((flags & THREE_BIT_MASK) << FLAGS_SHIFT),
...
pub fn unpack_face(face: TeknikPackedFace) -> Option<([u32; 3], u32, u8, u32, u32, u32)> {
    ...
    Some((plane_origin, direction, material, width, height, flags))
}
...
fn append_quad(
    ...
    let direction = direction_for(axis, face > 0);
    let axis_u = (axis + 1) % 3;
    let axis_v = (axis + 2) % 3;
    let plane_origin = [
        origin[0].round() as u32,
        origin[1].round() as u32,
        origin[2].round() as u32,
    ];
...
pub fn decode_packed_faces(
    ...
    for packed in packed_faces {
        let (plane_origin, direction, material, width, height, _flags) = unpack_face(*packed)?;
        let (axis, positive) = direction_axis(direction)?;
        let axis_u = (axis + 1) % 3;
        let axis_v = (axis + 2) % 3;
        let origin = [
            plane_origin[0] as f32,
            plane_origin[1] as f32,
            plane_origin[2] as f32,
        ];
...
mod tests {
    use super::*;
    use crate::terrain::{GRASS, PADDED_VOLUME, STONE};
        ...
        for y in 0..13_i32 {
            for z in 0..SIZE as i32 {
                for x in 0..SIZE as i32 {
                    padded[padded_index(x + 1, y + 1, z + 1)] = if y == 12 { GRASS } else { STONE };
                }
            }
        }
        padded[padded_index(7, 14, 9)] = STONE;
        // Neighbor-owned seam faces: outside solid, inside empty. These are
        // valid faces on plane 0 or 32 even though their owning voxel is not
        // part of the current chunk.
        padded[padded_index(4, 19, 0)] = GRASS;
        padded[padded_index(12, 21, SIZE as i32 + 1)] = STONE;
    ...
    #[test]
    fn packed_face_supports_planes_zero_and_32_with_full_extent() {
        let mut padded = vec![AIR; PADDED_VOLUME];
        ...
        for packed in &mesh.packed_faces {
            let (plane_origin, direction, material, width, height, flags) =
                unpack_face(*packed).expect("solid-cube face decodes");
            let (axis, _) = direction_axis(direction).unwrap();
            for (coordinate_axis, value) in plane_origin.into_iter().enumerate() {
                if coordinate_axis == axis {
                    assert!(value <= 32);
                } else {
                    assert!(value <= 31);
        ...
        assert_eq!(unpack_face(positive_x).unwrap().0[0], 32);
        let negative_x = mesh
            .packed_faces
            .iter()
            .copied()
            .find(|face| unpack_face(*face).unwrap().1 == FACE_NEG_X)
            .expect("negative X face exists");
        assert_eq!(unpack_face(negative_x).unwrap().0[0], 0);
    }
